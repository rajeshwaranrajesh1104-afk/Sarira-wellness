...
const DialogContent = ({ className, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div
        className={cn(
          "relative bg-background rounded-lg shadow-lg max-w-lg w-full max-h-[90vh] overflow-auto p-6 transition-opacity duration-300",
          className
        )}
      >
        <button
...
const DialogClose = ({ children, className }) => {
  const { setIsOpen } = useContext(DialogContext);
  return (
    <button type="button" onClick={() => setIsOpen(false)} className={cn(className)} aria-label="Close dialog">
      {children}
    </button>
  );
};
